...
def download_single_close_history(ticker: str, period_days: int) -> pd.Series:
    ...
    for attempt in range(1, DOWNLOAD_RETRIES + 1):
        try:
            ...
            # yfinance single ticker can return flat columns or multiindex
...
def download_core_close_history(tickers: List[str], period_days: int) -> Tuple[pd.DataFrame, List[str]]:
    """
    필수 티커는 아니지만 core universe 안의 ETF 실패는 건너뛴다.
    SPY/VIX/VIX3M 검증은 main()에서 따로 한다.
    """
...
def main() -> None:
    ...
    for ticker in required:
        if ticker not in core_closes.columns:
            raise RuntimeError(f"{ticker} data missing. Required core ticker download failed.")
