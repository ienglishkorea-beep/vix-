...
VIX_1D_JUMP_PCT = float(os.getenv("VIX_1D_JUMP_PCT", "15.0"))
VIX_3D_JUMP_PCT = float(os.getenv("VIX_3D_JUMP_PCT", "30.0"))
    ...
    for attempt in range(1, DOWNLOAD_RETRIES + 1):
        try:
            ...
            if data is None or data.empty or "Close" not in data.columns:
                raise RuntimeError(f"{ticker}: empty download or Close column missing")
...
def download_batch_close_history(tickers: List[str], period_days: int) -> pd.DataFrame:
    ...
    for batch in chunked(tickers, BREADTH_BATCH_SIZE):
        print(f"[INFO] Downloading batch: {batch}")
        for ticker in batch:
            try:
...
def main() -> None:
    ...
    core_tickers = [SPY_TICKER, VIX_TICKER, VIX3M_TICKER] + [x[0] for x in ETF_UNIVERSE]
    ...
    for ticker in core_tickers:
        s = download_single_close_history(ticker, HISTORY_DAYS)
    ...
    for ticker in required:
        if ticker not in core_closes.columns:
            raise RuntimeError(f"{ticker} data missing.")

    spy_close = core_closes[SPY_TICKER]
    vix_close = core_closes[VIX_TICKER]
    vix3m_close = core_closes[VIX3M_TICKER]
