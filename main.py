...
def get_sp500_tickers() -> List[str]:
    ...
    if not tables:
        raise RuntimeError("Failed to fetch S&P 500 constituents.")
    df = tables[0]
    tickers = df["Symbol"].astype(str).str.replace(".", "-", regex=False).tolist()
    return sorted(set(tickers))


def download_close_history(tickers: List[str], period_days: int = HISTORY_DAYS) -> pd.DataFrame:
    data = yf.download(
        tickers=tickers,
...
def build_message(report: SignalReport) -> str:
    ...
    lines.append(
        ...
        f"({report.breadth.above_50_count}/{report.breadth.valid_count})"
    )
    lines.append("")
    ...
    if report.signal_triggered and report.selected:
        ...
        for c in report.selected:
            lines.append(
                f"- {c.ticker} {c.name} | 종가 {c.close:.2f} | 당일수익률 {c.day_return_pct:.2f}% | "
