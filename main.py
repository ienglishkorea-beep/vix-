...
def download_stooq_series(ticker: str) -> pd.Series:
    last_err = None
    ...
    for attempt in range(1, DOWNLOAD_RETRIES + 1):
        try:
            ...
            if not expected_cols.issubset(set(df.columns)):
                raise RuntimeError(f"{ticker}: unexpected Stooq format")
            ...
            last_err = e
            print(f"[WARN] Stooq download failed for {ticker} (attempt {attempt}/{DOWNLOAD_RETRIES}): {e}")
            time.sleep(DOWNLOAD_SLEEP_SEC * attempt)

    raise RuntimeError(f"Failed to download {ticker} from Stooq: {last_err}")
...
def main() -> None:
    ...
    print(f"[INFO] Downloading core tickers from Stooq for {len(core_tickers)} symbols...")
    core_series = []
    failed_core = []

    for ticker in core_tickers:
        try:
            s = download_stooq_series(ticker)
            core_series.append(s)
            time.sleep(0.5)
        except Exception as e:
            print(f"[WARN] core ticker skipped: {ticker} | {e}")
            failed_core.append(ticker)

    if not core_series:
        raise RuntimeError("No core tickers downloaded successfully.")

    core_closes = pd.concat(core_series, axis=1).sort_index()

    if SPY_TICKER not in core_closes.columns:
        raise RuntimeError("SPY data missing. Required core ticker download failed.")

    print(f"[INFO] Downloading VIX from FRED ({FRED_VIX_SERIES})...")
    vix_close = download_fred_series(FRED_VIX_SERIES)

    print(f"[INFO] Downloading breadth sample from Stooq for {len(sp500)} symbols...")
    breadth_series = []
    for ticker in sp500:
        try:
            s = download_stooq_series(ticker)
            breadth_series.append(s)
            time.sleep(0.2)
        except Exception as e:
    ...
    breadth_source = pd.concat(breadth_series, axis=1).sort_index()
